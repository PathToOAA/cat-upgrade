...
const FailModal: React.FC<Props> = ({ open, level, need, have, onSave, onGiveUp }) => {
  if (!open) return null
  const canSave = have >= need
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[92%] max-w-md rounded-xl border border-neutral-700 bg-neutral-900 text-neutral-100 p-4 shadow-xl">
        ...
        <div className="flex gap-2">
          <button
            className={`flex-1 px-4 py-2 rounded-md font-semibold ${canSave ? 'bg-emerald-400 text-black' : 'bg-neutral-700 text-neutral-400 cursor-not-allowed'}`}
            disabled={!canSave}
            onClick={onSave}
          >
            방지권 사용해 살리기
